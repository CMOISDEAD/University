public class ArregloPersona {
  ...
  private static final String APELLIDODELTA = "Davila";
  ...
  /*
   * Punto 2: Edad maxima personas masculinas entre 10 - 40 años.
   * */

  int edadMaximaHombres() {
    int retorno = 0;
    for (int i = 0; i < this.listadoPersona.length; i++) {
      int EDAD = this.listadoPersona[i].getEdad();
      if (this.listadoPersona[i].getGenero() == Persona.MASCULINO && EDAD > 18 && EDAD < 40) {
        retorno = EDAD > retorno ? EDAD : retorno;
      }
    }

    System.out.println("[Edad maxima hombres] " + retorno);
    return retorno;
  }
  ...
  public String[] mujeresApellidoE() {
    ...
    for (int i = 0; i < this.listadoPersona.length; i++) {
      if (this.listadoPersona[i].getGenero() == Persona.FEMENINO
          && this.listadoPersona[i].getApellidos() == APELLIDODELTA) {
    ...
    for (int i = 0; i < retorno.length; i++) {
      if (retorno[i] != null) {
        System.out.println("[Apellido seleccionado] " + retorno[i]);
      }
    }
